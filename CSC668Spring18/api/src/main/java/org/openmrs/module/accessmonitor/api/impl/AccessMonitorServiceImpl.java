...
public class AccessMonitorServiceImpl extends BaseOpenmrsService implements AccessMonitorService {
    ...
    /**
     * Returns a number of records in given timeframe, seperated by interval. It
     ...
     * @return list of Ojbect in sequence of Date, Integer, Date, Integer,
     * etc... representing the number of system accesses at the specified time
    ...
    @Transactional(readOnly = true)
    @Override
    public List<ChartData> getSummedRecordList(Date start, Date end, Integer interval) throws IllegalArgumentException,
            APIException {
    ...
    /**
     * Returns a number of records in given timeframe, separated by interval and
     * user
     *
     * @param start the start of the time frame
     * @param end the end of the time frame
    ...
    @Transactional(readOnly = true)
    @Override
    public List<ChartData> getFilteredSummedRecordList(Date start, Date end, Integer interval) throws IllegalArgumentException,
            APIException {
        ...
        for (int i = 0; i < data.size(); ++i) {
            ...
            while (tempCal.before(startTime) || !tempCal.before(stopTime)) {
                ...
                if (!temp.isEmpty()) {
                    list.add(new ChartData(lastUserId, lastUserGiven, lastUserFamily,
                            startTime.getTime(), stopTime.getTime(), temp.size()));
